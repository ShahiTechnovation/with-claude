...
export const titles = {
  ...
  events: () => 'Claude events in India | Meetups, workshops and build days',
  event: (name: string, city: string) => `${name} in ${city} | ${site.wordmark}`,
  cities: () => 'Claude communities across India | The atlas',
  city: (name: string) => `Claude community in ${name} | Events, builders and projects`,
  builders: () => 'Claude builders in India | The open index',
  builder: (name: string, city: string) => `${name} | Claude builder in ${city}`,
  projects: () => 'Projects built with Claude in India | Made with Claude',
  project: (name: string, city: string) => `${name} | Built with Claude in ${city}`,
  useCases: () => 'How India uses Claude | Real workflows from the community',
  useCase: (name: string) => `${name} | ${site.wordmark}`,
